...
def ConsoleLog(message):
    now = time.clock()
    ac.console("AutoCam(%d): %s"%(now, message))
    ac.log("AutoCam(%d): %s"%(now, message))
    ...
    try:
        ...
        if os.path.isfile(INI_File):
            ...
            for carId in range(0, ac.getCarsCount(),1):
                ...
                if SettingsConfig.has_option(section, carName):
            ...
            if not order == "":
                ...
                for car in orderStrings:
                    if not car == "":
                        ...
                        #ConsoleLog("Driver %d in position %d named %s"%(int(car), position + 1, ac.getDriverName(int(car))))
                        position = position + 1
...
def canSwitch(carID):
    if ((not ac.isCarInPitlane(carID)) or (ac.isCarInPitlane(carID) and ac.getCarState(carID,acsys.CS.SpeedKMH) > minPitKMH) or (allDriversInPits == 1)) and not ac.getDriverName(carID) in skipDrivers and ac.isConnected(carID) and (ac.getCarState(carID, acsys.CS.RaceFinished) == 0 or allDriversFinished):
        if len(preferredDrivers) > 0:
            if not ac.getDriverName(carID) in preferredDrivers:
                #if any preferrredDrivers connectected then return False
                for carId in range(0,ac.getCarsCount()-1,1):
                    if ac.getDriverName(carId) in preferredDrivers and ac.isConnected(carId):
                        return False
        return True
    else:
        return False
...
def autoCam():
    ...
    carFound = -1
    #if we switch drivers force a camera change?
    driverSwitched = 0
            ...
            if driverSwitchingEnabled == 1: # True:
                        ...
                        for car in range(0,ac.getCarsCount() - 1):
                            if ac.isConnected(car) and ac.getCarState(car, acsys.CS.RaceFinished) == 0:
                                ...
                                if carDistance > totalDistance - 800.0 and carDistance > maxDistance:                                    
                                    ...
                                    if not ac.getFocusedCar() == car:
                                        #ConsoleLog("Car closest to finishing set to %s"%(ac.getDriverName(car)))
                                        carFound = car                           
                    except:
                        pass
                ...
                if bIsRace == True and ac.getCarState(0, acsys.CS.LapTime) == 0.0 and carFound < 0:
                    ...
                    if now - lastFocusSwitch > countdownFocusSwitch:
                        ConsoleLog("running the countdown logic, focus driver is %s"%(ac.getDriverName(ac.getFocusedCar())))
                                ...
                                if getPosition(car) > currentPosition:
                                    if carFound > 0:
                                        if getPosition(car) < getPosition(carFound):
                                            #ConsoleLog("Setting car in position %d = Driver %s"%(getPosition(car), ac.getDriverName(car)))
                                            carFound = car                                        
                                    else: 
                                        #ConsoleLog("Setting car in position %d = Driver %s"%(getPosition(car), ac.getDriverName(car)))
                                        carFound = car
                        if carFound < 0:
                            ...
                            for car in range(0,ac.getCarsCount() - 1):
                                if ac.isConnected(car) and not ac.isCarInPitlane(car):
                                    if carFound > 0:
                                        if getPosition(car) < getPosition(carFound):
                                            ConsoleLog("Resetting car in position %d = Driver %s"%(getPosition(car), ac.getDriverName(car)))
                                            carFound = car
                                            countdownFocusSwitch = 10.0
                                    else: 
                                        ConsoleLog("Initializing car in position %d = Driver %s"%(getPosition(car), ac.getDriverName(car)))
                                        carFound = car
                                        countdownFocusSwitch = 10.0
                ...
                if ac.getCarState(0, acsys.CS.LapTime) > 0.0 and anyDriverFinished == 0: #KMH and Pit Lane tracking
                    for car in range(0,ac.getCarsCount() - 1):
                        if ac.isConnected(car) and (not ac.isCarInPitlane(car) or ac.getCarState(car,acsys.CS.SpeedKMH) > minPitKMH):
                            ...
                            driverName = ac.getDriverName(car)
                            ...
                            if speedKMH < 400.0:
                                ...
                                if offPaceCar < 0 and bIsRace and ac.isCarInPitlane(car) and ac.getCarState(car,acsys.CS.SpeedKMH) > minPitKMH and now - lastFocusSwitch > minSwitchDelay: # and speedKMH < 90.0: # and lapCount > 0:\
                                    #ConsoleLog("Driver %s driving in pit lane driving %0.2fKMH"%(ac.getDriverName(car), ac.getCarState(car,acsys.CS.SpeedKMH)))
                                    tmpKey = "Car%dOffPaceTime"%(car)
                                    if tmpKey in dic:
                                        if now - dic[tmpKey] > offPaceSwitchDelay:
                                            ConsoleLog("Driver %s driving in pit lane driving %0.2fKMH"%(ac.getDriverName(car), ac.getCarState(car,acsys.CS.SpeedKMH)))
                                    ...
                                    if False: #WE ARE NOT DOING THIS FOR NOW, TRYING OTHER LOGIC INSTEAD
                                        ...
                                        if strKey in dicPitEntry:
                                            ...
                                            #ConsoleLog("Driver %s distance = %0.2f"%(ac.getDriverName(car), distance))
                                            if distance < 5.0:
                                                ConsoleLog("Driver %s driving towards pit entry"%(ac.getDriverName(car)))
                                                offPaceCar = car
                                                setCamera = 2
                        ...
                        #        ConsoleLog("Clearing offPace timestamp for %s, %0.2fKMH"%(ac.getDriverName(car), ac.getCarState(car,acsys.CS.SpeedKMH)))
                        #        del dic[tmpKey]
                ...
                if len(preferredDrivers) > 0 and carFound > -1:
                    if not ac.getDriverName(carFound) in preferredDrivers:
                        carFound = -1
                ...
                if offPaceCar >= 0: # and not offPaceCar == lastOffPaceCar:
                    if now - lastFocusSwitch > driverSwitchDelay or (ac.isCarInPitlane(offPaceCar) and (ac.getCarState(offPaceCar,acsys.CS.SpeedKMH) < minPitKMH)):
                        ConsoleLog("Clearing offPaceCar, Driver = %s, InPitLane = %d, KMH = %.2f"%(ac.getDriverName(offPaceCar), ac.isCarInPitlane(offPaceCar),ac.getCarState(offPaceCar,acsys.CS.SpeedKMH)))
                        offPaceCar = -1
                    elif not offPaceCar == ac.getFocusedCar(): # and not offPaceCar == lastOffPaceCar:
                        ConsoleLog("Switching to offPaceCar Driver %s"%(ac.getDriverName(offPaceCar)))
                        lastFocusSwitch = now
                    ...
                    if bIsQually == True:
                        if now - lastFocusSwitch > quallySwitchDelay:
                            ConsoleLog("using qually optional logic to switch to %s"%(ac.getDriverName(carFound)))
                        ...
                        ConsoleLog("using regular optional logic to switch to %s"%(ac.getDriverName(carFound)))
                        ac.focusCar(carFound)
                        driverSwitched = 1
                        lastFocusSwitch = now                                                            
                ...
                if len(preferredDrivers) > 0:
                    if not ac.getDriverName(ac.getFocusedCar()) in preferredDrivers:
                        for carId in range(0,ac.getCarsCount()-1,1):
                            if ac.getDriverName(carId) in preferredDrivers and ac.isConnected(carId):
                                ConsoleLog("Forcing Preferred Driver %s"%(ac.getDriverName(carId)))
                                ac.focusCar(carId)
                                driverSwitched = 1
                                return True
            

        #MOVE CAM SWITCHING TO HERE
        if AutoCamActive:
            if cameraSwitchingEnabled == 1:
                now = time.clock()
                countdownCam = 5
                ...
                if bIsRace and ac.getCarState(0, acsys.CS.LapTime) == 0.0 and ac.isCarInPitlane(0) and not ac.getCameraMode() == countdownCam:
                    ...
                    ConsoleLog("Forcing Camera to %d, order is %s"%(countdownCam, order))
                    #force the F5 cam?
                    ...
                    if driverSwitchingEnabled == 1:
                        ...
                        for car in range(0,ac.getCarsCount() - 1):
                            if ac.isConnected(car) and not ac.isCarInPitlane(car):
                                ConsoleLog("Driver %s is in position %d"%(ac.getDriverName(car), getPosition(car)))
                                if carFound > 0:
                                    if getPosition(car) < getPosition(carFound):
                                        #ConsoleLog("Resetting car in position %d = Driver %s"%(getPosition(car), ac.getDriverName(car)))
                                        carFound = car
                ...
                if ac.isCarInPitlane(ac.getFocusedCar()):
                    ...
                    if usingPitCam == 0:
                        ...
                        ConsoleLog("Forcing defaultPitCam %d for %s"%(defaultPitCam, ac.getDriverName(ac.getFocusedCar)))
                        setCamera = defaultPitCam
                ...
                if len(cameraSwitching) > 0 and len(firstLapSwitching) > 0 and (not bIsRace or ac.getCarState(0, acsys.CS.LapTime) > 0.0):
                    ...
                    if now - cameraSwitchDelay > cameraSwitchTimer or driverSwitched == 1: # or not canSwitch(ac.getFocusedCar()):
                        ...
                        if ac.isCarInPitlane(ac.getFocusedCar()):
                            ...
                            intGuess = random.randint(1, len(pitCameraSwitching))
                            tmpKey = "Guess%d"%(intGuess)
                            if tmpKey in pitCameraSwitching:
                                ...
                                if True: # tmpKey in cameraSwitching:                        
                                    ...
                                    if nextCam == 0 and currentCam == 0:
                                        ...
                                        if True:
                                            ...
                                            intGuess = random.randint(1, len(dicCars[carName]))
                                            tmpKey = "Guess%d"%(intGuess)
                                            if verbose == 1:
                                                ConsoleLog("Car Camera tmpkey = %s"%(tmpKey))
                                            randomCamInt = dicCars[carName][tmpKey] #    random.randint(0,carCamCount)
                                            ...
                                            randomCamInt = random.randint(0,carCamCount)
                                            ac.setCameraCar(randomCamInt,currentId)                      
                                            setCamera = nextCam
                                            if verbose == 1:
                                                ConsoleLog("carCamera is set to %d"%(randomCamInt))
