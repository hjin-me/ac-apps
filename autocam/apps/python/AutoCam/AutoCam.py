...
def buildHudWindow():
    ...
    # two-row horizontal banner: row 1 = driver / car / rank, row 2 = lap times / gaps
    hudName = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudName, 14, 8)
    ac.setFontSize(hudName, 22)
    setLabelColor(hudName, *COLOR_GOLD)

    hudCar = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudCar, 210, 12)
    ac.setFontSize(hudCar, 18)

    hudPos = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudPos, 440, 12)
    ac.setFontSize(hudPos, 18)

    hudLast = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudLast, 14, 48)
    ac.setFontSize(hudLast, 18)

    hudBest = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudBest, 190, 48)
    ac.setFontSize(hudBest, 18)

    hudGapAhead = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudGapAhead, 360, 48)
    ac.setFontSize(hudGapAhead, 18)

    hudGapBehind = ac.addLabel(hudWindow, "--")
    ac.setPosition(hudGapBehind, 530, 48)
    ac.setFontSize(hudGapBehind, 18)
